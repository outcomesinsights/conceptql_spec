...
- [ConceptQL Specification](#conceptql-specification)
  ...
  - [Inline-Filter Operators](#inline-filter-operators)
    - [Filter by Place of Service](#filter-by-place-of-service)
    - [Filter by Provenance](#filter-by-provenance)
    - [Filter by Provider (Specialty)](#filter-by-provider-specialty)
  ...
  - [Gathering Coincident Events](#gathering-coincident-events)
...
At its inception, ConceptQL was developed to query [OMOP's CDM v4.0](https://github.com/OHDSI/CommonDataModel/releases/tag/v4) and its associated tables.  Each table represented a certain "type" of data (e.g. condition_occurrence, procedure, visit_occurrence, etc) and ConceptQL was designed to treat each type of data as distinct from the other types.
...
- Takes any number of upstream operators and aggregates their streams
    ...
    - Streams with the different types flow along together concurrently without interacting
        - It does not make sense to union, say, condition_occurrences with procedures, so streams with different types won't mingle together, but will continue to flow downstream in parallel
...
**Example 7 - Two streams of the same type (condition_occurrence) joined into a single stream, then a different stream (procedure) flows concurrently**
...
![Two streams of the same type (condition_occurrence) joined into a single stream, then a different stream (procedure) flows concurrently](README/07520638f7e898be1bebb8ab6fb6eb824024fdc97a25747f62684c4389afc0d0.png)
...
**Example 8 - Two streams of the same type (condition_occurrence) joined into a single stream, along with a different stream (procedure) flows concurrently (same as above example)**
...
![Two streams of the same type (condition_occurrence) joined into a single stream, along with a different stream (procedure) flows concurrently (same as above example)](README/3399e13ce4d77609ef48dd14ad9f20cb3d4a591f13259d63275608c9a515a617.png)
...
**Example 12 - All Conditions that are MI unless they are primary diagnoses**
...
![All Conditions that are MI unless they are primary diagnoses](README/9816c700006428e6ee95e5ed7bc716e06e7d323ee2c3f06c29237d8c0687b6fd.png)
...
Sometimes it is difficult to reason through `time_window` when working with temporal comparison operators.  It would be nice if a more intuitive language could be used to describe some common temporal relationships.
...
![](README/0e6739d7a5e4b296341a35177f9fd14c7664a85fa78dbb9830fc61dd95401f96.png)
...
![](README/808768e04b813f7184a6ac9274fa673d97815dca0cb34202135abdebfa95d248.png)
...
There are rare occassions when we'd like to stitch a set of events together into a span of time, such as a set of prescriptions events into a span of time we'd consider a patient to be taking a certain medication.  ConceptQL provides the `episode` operator for such an occasion.
...
There are a couple of operators that can filter an incoming stream.

### Filter by Place of Service

Often in claims data, an event is reported with a place of service via the CMS place of service codes, such as "inpatient hostpital" (21) or "outpatient hospital" (22).  For these kinds of queries, ConceptQL has the `place_of_service_filter`
...
There are times when we'd like to captures events only when a particular provider specialty was involved.  ConceptQL provides the `provider_filter` operator for this.  This operator is a bit bare-bones at the moment in that it requires users to enter in the concept ID of the specialty they are seeking, rather than providing a list of known specialties.
...
## Gathering Coincident Events

Claims data often reports diagnoses on the same "line" as a procedure.  This is done for billing reasons, essentially the provider is saying "I preformed this procedure due to these conditions present in the pateint".  ConceptQL has an operator that specifically targets this kind of relationship: `co_reported`
...
**Example 46 - Diabetes diagnosis reported as part the resaon for an office visit**
...
![Diabetes diagnosis reported as part the resaon for an office visit](README/dfc63bee1d9f447450c764cc505def315209de4c752ce49f4cb3779dd28b1001.png)
...
| read | condition_occurrence | 1 or more read codes | All results whose codes match any of the codes  that match the Read codes entered.  See note below on the Read operator |
